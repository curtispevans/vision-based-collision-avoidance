...
for i in range(25, 525):
    ...
    if i % 5 == 0:
        for i, wedge in enumerate(wedges):
            vertices = wedge.get_wedge_vertices(sim_time)
            dict_of_vertices[i].append(vertices)
        
        if plot:
            ax.clear()
            for i in range(3):
                utils.plot_wedge(dict_of_vertices[i][-1], ax)
...
print(f'Saved the list of vertices in {round(time.time() - start,2)} seconds')
