...
def plan_path(bearing_angles, sizes, ownship_poses, num_intruders, safety_threshold=-50) -> NDArray:
    ...
    for i in range(num_intruders):
        wedge = ppu.create_wedge(bearing_angles[i][:params.measured_window], sizes[i][:params.measured_window], ownship_poses[:params.measured_window])
        wedges.append(wedge)

    in_out_wedge_list, list_of_vertices = ppu.get_in_out_wedges_and_vertices(wedges, plotting=True)
